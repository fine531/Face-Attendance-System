    ...
    if request.method == 'POST':
        ...
        if image_data:
            ...
            if is_valid_email(email):
                ...
                text_content = (
                    f"Hello {name},\n\n"
                    f"Congratulations! Your registration for class {student_class} was successful.\n\n"
                    "Thank you for joining us. If you have any questions, feel free to reply to this email.\n\n"
                    "Best regards,\nThe Face Attendance System Team"
                )
                html_content = f"""
                <html>
                  <body>
                    <h2 style='color:#007bff;'>Face Attendance System</h2>
                    <p>Hello {name},</p>
                ...
                """
                try:
                    send_html_email(subject, email, text_content, html_content)
                    message = "Registration successful. A confirmation email has been sent."
                except Exception as e:
                    logging.error(f"Failed to send registration email to {email}: {e}")
                    message = "Registration successful, but we could not send a confirmation email."
            else:
                message = "Registration successful. (No confirmation email sent.)"

            return render(request, 'selfie_success.html', {'message': message, 'name': name})
...
def student_authorize(request, pk):
    ...
    if request.method == 'POST':
        ...
        student.save()
        # Send email only if authorized is True and email is valid
        if student.authorized and is_valid_email(student.email):
            subject = "You're Now Authorized!"
            text_content = (
                f"Hello {student.name},\n\n"
                f"Good news! You have been authorized for class {student.student_class}. You can now access all features of the Face Attendance System.\n\n"
            ...
            <html>
              <body>
                ...
                <p>Good news! You have been authorized for class <b>{student.student_class}</b>. You can now access all features of the Face Attendance System.</p>
                <p>If you have any questions, just reply to this email.<br>
            ...
            """
            try:
                send_html_email(subject, student.email, text_content, html_content)
                message = "Student authorized and confirmation email sent."
            except Exception as e:
                logging.error(f"Failed to send authorization email to {student.email}: {e}")
                message = "Student authorized, but could not send confirmation email."
        else:
            message = "Student authorized. (No confirmation email sent.)"
...
def export_students(request):
    ...
    writer.writerow(['Name', 'Email', 'Phone Number', 'Class', 'Check-In Time', 'Check-Out Time', 'Date Registered'])
    for student in Student.objects.all():
        writer.writerow([
            student.name,
            student.email,
            student.phone_number,
            getattr(student, 'class_name', getattr(student, 'student_class', '')),
            getattr(student, 'check_in_time', ''),
            getattr(student, 'check_out_time', ''),
            getattr(student, 'created_at', getattr(student, 'date_joined', '')),
        ])
    return response
